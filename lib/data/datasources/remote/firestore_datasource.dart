...
class FirestoreDatasource {
  ...
  Stream<Map<String, dynamic>?> watchData(String collection, String docId) {
    return _firestore.collection(collection).doc(docId).snapshots().map((doc) => doc.data());
  }

  Stream<List<Map<String, dynamic>>> watchCollection(String collection, {String? userId}) {
    Query query = _firestore.collection(collection);
    if (userId != null) {
      query = query.where('userId', isEqualTo: userId);
    }
